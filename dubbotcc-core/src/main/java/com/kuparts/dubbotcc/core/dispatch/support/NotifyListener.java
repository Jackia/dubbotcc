...
public abstract class NotifyListener implements Notify {
     ...
     * @param command 命令
     */
    public boolean notify(final ActorType type, final InvokeCommand command, final List<Actor> actors, NotifyCallback... callbacks) {
        try {
            CommandType commandType = getCommandType(type, actors);
            ...
            command.setTargetActor(actors);
            return execute(actor, command, callbacks);
     ...
     * @param command   命令
     */
    public void asyncNotify(final ActorType type, final InvokeCommand command, final List<Actor> actors, NotifyCallback... callbacks) {
        CompletableFuture.supplyAsync(new BooleanSupplier(type, command, actors)).thenAccept(flag -> {
            if (flag) {//通知成功
    ...
    /**
     * 获取命令类型,根不同的属性判断,可能发送的命令
     *
     * @param actors 根据变化节点
     * @return 命令类型
     */
    private CommandType getCommandType(final ActorType type, List<Actor> actors) {
        Optional<Actor> optional;
        switch (type) {
            case MASTER:
                optional = actors.stream().filter(e -> e.getType() == ActorType.OBSERVE).findFirst();
                if (optional.isPresent()) {
                    return CommandType.SLAVE_OBSERVE;
                }
                optional = actors.stream().filter(e -> e.getType() == ActorType.SLAVE).findFirst();
                if (optional.isPresent()) {
                    return CommandType.SLAVE_EX;
                }
                break;
            case OBSERVE:
                optional = actors.stream().filter(e -> e.getType() == ActorType.MASTER).findFirst();
                if (optional.isPresent()) {
     ...
     * 模版方法法,回调前的执行方法
     *
     * @param targetActor 接收者
     * @param command     执行指令
     * @param callbacks   执行完成后的回调
     * @return
     */
    protected abstract boolean execute(Actor targetActor, InvokeCommand command, NotifyCallback... callbacks);
    ...
    private class BooleanSupplier implements Supplier<Boolean> {
        ...
        public BooleanSupplier(ActorType type, InvokeCommand command, final List<Actor> actors, NotifyCallback... callbacks) {
            this.type = type;
